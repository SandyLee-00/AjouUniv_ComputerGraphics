...
void init() {
    ...
    for (int i = 0; i < 360; i++) {
        radian = i + 3.14159265358979323846 / 180.0;
        triVertData.push_back(vec3(cos(radian), sin(radian), 0));
    }
    vector<u16vec3> triData;
    for (int i = 1; i < 361; i++) {
        if (i == 360) {
            triData.push_back(u16vec3(0, i, 1));
        }
...
void render(GLFWwindow* window) {
    ...
    glBindVertexArray(vertexArrayID);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexVBOID);
    glDrawElements(GL_TRIANGLES, 1080, GL_UNSIGNED_SHORT, 0);
